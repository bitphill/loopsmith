 ...
 *
 * Ported from beautifului.dev's GlideMenu primitive and re-skinned onto Forge:
 * the highlight defaults to `bg-raised` (Forge has no `hover` token). Rows are
 * marked with `data-menu-row`; the layer measures the row under the pointer (or
 * focus) and animates its own box to match. Logic is unchanged from the source.
 */
...
export default function GlideMenu({
  children,
  className = "",
  highlightClassName = "inset-x-0 rounded-[10px] bg-raised",
