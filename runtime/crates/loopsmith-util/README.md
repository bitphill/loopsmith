<div align="center">
  <img src="https://raw.githubusercontent.com/bitphill/loopsmith/v0.3.0/assets/loopsmith-logo-256.png" alt="loopsmith" width="140" />
  <h1>loopsmith-util</h1>
  <p><em>Dependency-free primitives every other loopsmith crate needs.</em></p>
</div>

[![crates.io](https://img.shields.io/crates/v/loopsmith-util?logo=rust&logoColor=white&label=crates.io&color=e6522c)](https://crates.io/crates/loopsmith-util)
...
No dependencies at all, not even `serde`. Every other crate in the workspace
...
- **`which`** — resolve a command the way a shell would. A candidate counts only
  if it is executable, and on Windows each `PATHEXT` suffix is tried, appended
  rather than substituted so `check.sh` may become `check.sh.exe`.
- **`now_ms`** — the single clock. Timestamps are stored as numbers so the ledger
  stays sortable without a date parser.
- **`platform`** — what the host *actually* is, decided at run time rather than
  at build time: which bash, GNU or BSD userland, which scheduler is installed,
  and the home directory by whichever variable this OS uses. `cfg!(target_os)`
  answers none of those, and every one of them changes what a generated script
  may assume. The userland is probed by asking `sed` for a version, because
  Homebrew's coreutils puts GNU tools ahead of BSD ones on a Mac.
